...
function SidePanel(): ReactElement {
  ...
  return <main className="shell side"><Brand/>
    <section className="panel stack">
      <div><span className="pill">Current page context</span><p className="muted">{context ? context.title : 'Click refresh if the page context is unavailable.'}</p></div>
      {targetElement && <div className="panel stack"><span className="pill">Targeted element</span><p className="muted"><strong>{targetElement.selector}</strong></p><p className="tiny muted">{targetElement.text || targetElement.role}</p><button className="ghost" onClick={clearTargetElement}>Clear element target</button></div>}
      <textarea value={prompt} onChange={(e)=>{ setPrompt(e.target.value); setConsent(false); }} placeholder="Describe how this site should change..."/>
      <label className="tiny muted">Context sent to AI provider<select value={contextMode} onChange={(e)=>{ setContextMode(e.target.value as ContextMode); setConsent(false); }} disabled={Boolean(targetElement)}><option value="page">Current page summary</option><option value="selected-text">Selected text only</option><option value="minimal">Minimal: URL and title only</option></select></label>
      <div className="panel stack"><strong>Privacy preview</strong><p className="tiny muted">{contextPreview}</p><label className="tiny muted"><input type="checkbox" checked={consent} onChange={(e)=>setConsent(e.target.checked)}/> I approve sending this prompt and context to my configured AI provider.</label></div>
      <div className="row"><button disabled={busy || !prompt.trim()} onClick={runGenerate}>{busy ? 'Generating…' : targetElement ? 'Generate for selected element' : 'Generate modification'}</button><button className="secondary" disabled={picking} onClick={pickElement}>{picking ? 'Pick on page…' : 'Select element'}</button><button className="secondary" onClick={()=>getPageContext().then((page)=>{ setContext(page); setConsent(false); })}>Refresh context</button></div>
...
function contextForMode(page: PageContext, mode: ContextMode, targetElement: SelectedElement | null): PageContext {
  if (targetElement) return { ...page, targetElement, selectedText: '', visibleTextSample: '', headings: [], elementSummary: [{ selector: targetElement.selector, role: targetElement.role, text: targetElement.text }] };
...
function describeContext(page: PageContext | null, mode: ContextMode, targetElement: SelectedElement | null): string {
  ...
  if (targetElement) return `Prompt + URL/title + selected element only: ${targetElement.selector}. Element text preview: ${targetElement.text.slice(0, 160) || 'none'}.`;
  if (mode === 'selected-text') return `Prompt + URL/title + browser-selected text only (${page.selectedText.length} characters). No visible page text or element list.`;
