...
import { deleteModification, generate, getPageContext, getState, saveModification, toggleModification } from './client';
...
function Popup(): ReactElement {
  ...
  return <main className="shell"><Brand/><section className="panel stack"><button onClick={() => chrome.sidePanel.open({ windowId: chrome.windows.WINDOW_ID_CURRENT })}>Open FlexWeb Studio</button><button className="secondary" onClick={() => chrome.runtime.openOptionsPage()}>Options & Library</button>{error && <p className="danger">{error}</p>}</section><section className="panel"><ModificationList modifications={mods} onToggle={(id,en)=>toggleModification(id,en).then(setMods)} onDelete={(id)=>deleteModification(id).then(setMods)} onSave={(mod)=>saveModification(mod).then(setMods)} onRegenerate={async (prompt)=>generate(prompt, await getPageContext())}/></section></main>;
}
